...
export default function Movies({ token }) {
  ...
  useEffect(async () => {
    if (token) {
      const Fav = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/movies`, {
        headers: { authorization: "Bearer " + token },
      });
      setFavorit(Fav.data);
  ...
  const deleteMovies = async (id, index) => {
    ...
    //   الكوبي هي الي فيها الافلام الحاليه
    // لانها state  مقدر اعدل فيها نفسها
    copiedArr.splice(index, 1);
    // فاسوي لها نسخه ثم ارجع احطها هي state
    setmovies(copiedArr);
  };
  ...
  const toggleColor = async (id) => {
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/Favorit/${id}`,
        {},
        { headers: { authorization: "Bearer " + token } }
      );
  ...
  const updatenam = async (id, i) => {
    ...
    const updateMovies = await axios.put(
      `${process.env.REACT_APP_BACKEND_URL}/updet`,
      {
        id: id,
